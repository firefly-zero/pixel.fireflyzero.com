...
class Tool {
  ...
  constructor(name, options) {
    ...
    if (this.mainButton != undefined) {
      ...
      Events.on(
        ...
        this.mainButton,
        function () {
          if (this.tutorialTimer != undefined) clearTimeout(this.tutorialTimer);
  ...
  showTutorial() {
    ...
    if (
      this.mainButton.getBoundingClientRect().top -
        48 +
        (tutorialRect.bottom - tutorialRect.top) >
      window.innerHeight
    ) {
      this.toolTutorial.style.top =
        window.innerHeight -
        48 -
        (tutorialRect.bottom - tutorialRect.top) +
        "px";
      ...
      this.toolTutorial.style.top =
        this.mainButton.getBoundingClientRect().top - 48 + "px";
    }
  ...
  onHover(cursorLocation, cursorTarget) {
    ...
    this.brushPreview.style.left =
      Math.floor(cursorLocation[0] / currFile.zoom) * currFile.zoom +
      currFile.currentLayer.canvas.offsetLeft -
      (this.currSize * currFile.zoom) / 2 -
      currFile.zoom / 2 +
      toSub * currFile.zoom +
      "px";
    this.brushPreview.style.top =
      Math.floor(cursorLocation[1] / currFile.zoom) * currFile.zoom +
      currFile.currentLayer.canvas.offsetTop -
      (this.currSize * currFile.zoom) / 2 -
      currFile.zoom / 2 +
      toSub * currFile.zoom +
      "px";
    ...
    if (this.cursorType.type == "html") {
      if (
        cursorTarget.className == "drawingCanvas" ||
        cursorTarget.className == "drawingCanvas"
      ) {
