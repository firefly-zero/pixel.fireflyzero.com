const EditorState = (() => {
  ...
  function switchMode(newMode, skipConfirm = false) {
    ////console.trace();
    const switchText =
      "Switching to basic mode will flatten all the visible layers. Are you sure you want to continue?";
    if (
      !firstFile &&
      newMode == "Basic" &&
      !skipConfirm &&
    ...
    if (newMode == "Advanced") {
      ...
      document.getElementById("switch-mode-button").innerHTML =
  ...
  function chooseMode() {
    ...
    //change splash text
    document
      .querySelector("#sp-quickstart-container .mode-switcher")
      .classList.remove(prevMode + "-mode");
    document
      .querySelector("#sp-quickstart-container .mode-switcher")
      .classList.add(pixelEditorMode.toLowerCase() + "-mode");
  }

  function toggleMode() {
    if (pixelEditorMode == "Advanced") switchMode("Basic");
    else switchMode("Advanced");
  }
