...
const ColorModule = (() => {
  ...
  document
    ...
    .addEventListener("change", colorChanged, false);
  document
    .getElementById("jscolor-hex-input")
  ...
  Events.on("click", document.getElementById("cm-zoomout"), resizeSquares, {
    altKey: true,
    deltaY: 1.0,
  });
  ...
  function addColor(newColor) {
    ...
    if (newColor.charAt(0) != "#") newColor = "#" + newColor;
    ...
    newEditButton.addEventListener("click", (event) => {
      ...
      else Dialogue.showDialogue("palette-block", false);
    });
  ...
  function replaceAllOfColor(oldColor, newColor) {
    ...
    var tempImage = currFile.currentLayer.context.getImageData(
      0,
      0,
    ...
    for (var i = 0; i < tempImage.data.length; i += 4) {
      ...
      if (
        tempImage.data[i] == oldColor.r &&
        tempImage.data[i + 1] == oldColor.g &&
        tempImage.data[i + 2] == oldColor.b
      ) {
        //change to new color
  ...
  function createColorPalette(paletteColors, clearCurrent = true) {
    ...
    if (clearCurrent) {
      while (colorsMenu.childElementCount > 1) colorsMenu.children[0].remove();
    }
    ...
    for (var i = 0; i < paletteColors.length; i++) {
      ...
      if (
        newColRgb.r + newColRgb.g + newColRgb.b >
        lightestColorRgb.r + lightestColorRgb.g + lightestColorRgb.b
      )
  ...
  function updateCurrentColor(color, refLayer) {
    ...
    if (refLayer) color = refLayer.context.fillStyle;
