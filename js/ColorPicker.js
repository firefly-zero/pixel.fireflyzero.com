const ColorPicker = (() => {
  ...
  let hexContainers = [
    document.getElementById("cp-colours-previews").children[0],
    null,
    null,
    null,
  ];
  let startPickerIconPos = [
    [0, 0],
    [0, 0],
    [0, 0],
    [0, 0],
  ];
  let currPickerIconPos = [
    [0, 0],
    [0, 0],
    [0, 0],
    [0, 0],
  ...
  function init() {
    ...
    startPickerIconPos[0] = [
      miniPickerCanvas.getBoundingClientRect().left,
      miniPickerCanvas.getBoundingClientRect().top,
    ];
    ...
    switch (index) {
      case 1:
        ...
        switch (currentPickerMode) {
          case "rgb":
            gradientMin =
              "rgba(0," + sliderValues[1] + "," + sliderValues[2] + ",1)";
            gradientMax =
              "rgba(255," + sliderValues[1] + "," + sliderValues[2] + ",1)";
            break;
          case "hsv":
      ...
      case 2:
        ...
        switch (currentPickerMode) {
          case "rgb":
            gradientMin =
              "rgba(" + sliderValues[0] + ",0," + sliderValues[2] + ",1)";
            gradientMax =
              "rgba(" + sliderValues[0] + ",255," + sliderValues[2] + ",1)";
            break;
          case "hsv":
            rgbColour = Color.hsvToRgb({
              h: sliderValues[0],
              s: 0,
              v: sliderValues[2],
            });
      ...
      case 3:
        ...
        switch (currentPickerMode) {
          case "rgb":
            gradientMin =
              "rgba(" + sliderValues[0] + "," + sliderValues[1] + ",0,1)";
            gradientMax =
              "rgba(" + sliderValues[0] + "," + sliderValues[1] + ",255,1)";
            break;
          case "hsv":
            rgbColour = Color.hsvToRgb({
              h: sliderValues[0],
  ...
  function changePickerMode(newMode) {
    ...
    document
      .getElementsByClassName("cp-selected-mode")[0]
      .classList.remove("cp-selected-mode");
  ...
  function getSlidersValues() {
    return [
      parseInt(sliders[0].getElementsByTagName("input")[0].value),
  ...
  function updateSlidersByHex(hex, updateMini = true) {
    ...
    let mySliders = [
      sliders[0].getElementsByTagName("input")[0],
  ...
  function updatePickerByHex(hex) {
    ...
    if (currPickerIconPos[0][1] >= 92) currPickerIconPos[0][1] = 91.999;
  ...
  function getMiniPickerColour() {
    ...
    pickedColour = miniPickerCanvas
      .getContext("2d")
      .getImageData(
        currPickerIconPos[0][0] + 8,
        currPickerIconPos[0][1] + 8,
        1,
        1,
      ).data;
    ...
    return new Color(
      "rgb",
      Math.round(pickedColour[0]),
      Math.round(pickedColour[1]),
      Math.round(pickedColour[2]),
    ).hex;
  }
  ...
  function updateMiniSlider(hex) {
    ...
    styles[1] =
      "input[type=range]#cp-minipicker-slider::-webkit-slider-runnable-track { background: rgb(2,0,36);";
    styles[1] +=
      "background: linear-gradient(90deg, rgba(2,0,36,1) 0%, rgba(0,0,0,1) 0%, " +
      "rgba(" +
      rgb.r +
      "," +
      rgb.g +
    ...
    styles[1] +=
      "input[type=range]#cp-minipicker-slider::-moz-range-track { background: rgb(2,0,36);";
    styles[1] +=
      "background: linear-gradient(90deg, rgba(2,0,36,1) 0%, rgba(0,0,0,1) 0%, " +
      "rgba(" +
      rgb.r +
  ...
  function updateMiniPickerSpectrum() {
    ...
    let white = new Color("hsv", hsv.h, 0, parseInt(miniPickerSlider.value))
      .rgb;
    ...
    vGrad.addColorStop(
      0,
      "rgba(" + white.r + "," + white.g + "," + white.b + ",0)",
    );
    vGrad.addColorStop(
      1,
  ...
  function changePickingMode(newMode, event) {
    ...
    document
      .getElementById("cp-colour-picking-modes")
      .getElementsByClassName("cp-selected-mode")[0]
      .classList.remove("cp-selected-mode");
    // Updating mode
    ...
    for (let i = 1; i < nIcons; i++) {
      ...
      hexContainers[0].parentElement.removeChild(
        hexContainers[0].parentElement.children[1],
      );
  ...
  function updateOtherIcons() {
    ...
    switch (currentPickingMode) {
      ...
      case "analog":
        // First colour
        newColourHsv = new Color(
          "hsv",
          (currentColourHsv.h + 40) % 360,
          currentColourHsv.s,
          currentColourHsv.v,
        );

        currPickerIconPos[1][0] =
          (miniPickerCanvas.width * newColourHsv.hsv.h) / 360 - 8;
        currPickerIconPos[1][1] =
          miniPickerCanvas.height -
          ((miniPickerCanvas.height * newColourHsv.hsv.s) / 100 + 8);
        ...
        newColourHsv = new Color(
          "hsv",
          (currentColourHsv.h + 320) % 360,
          currentColourHsv.s,
          currentColourHsv.v,
        );

        currPickerIconPos[2][0] =
          (miniPickerCanvas.width * newColourHsv.hsv.h) / 360 - 8;
        currPickerIconPos[2][1] =
          miniPickerCanvas.height -
          ((miniPickerCanvas.height * newColourHsv.hsv.s) / 100 + 8);
      ...
      case "tri":
        for (let i = 1; i < 3; i++) {
          newColourHsv = new Color(
            "hsv",
            (currentColourHsv.h + 120 * i) % 360,
            currentColourHsv.s,
            currentColourHsv.v,
          );

          currPickerIconPos[i][0] =
            (miniPickerCanvas.width * newColourHsv.hsv.h) / 360 - 8;
          currPickerIconPos[i][1] =
            miniPickerCanvas.height -
            ((miniPickerCanvas.height * newColourHsv.hsv.s) / 100 + 8);
    ...
    if (currentPickingMode != "analog") {
      ...
      for (let i = 0; i < pickerIcons.length - 1; i++) {
        ...
        hexContainers[i + 1].style.color = getHexPreviewColour(
          newColourHexes[i],
        );
      }
    }
    // If I'm using analogous mode, I place the current colour in the middle
      ...
      for (let i = 1; i < 3; i++) {
        hexContainers[i].style.color = getHexPreviewColour(
          newColourHexes[i - 1],
        );
      }
    }
