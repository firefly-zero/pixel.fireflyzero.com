...
const FileManager = (() => {
  ...
  const browsePaletteHolder = document.getElementById(
    "load-palette-browse-holder",
  );
  const importImageHolder = document.getElementById(
    "import-image-browse-holder",
  );
  ...
  function openSaveProjectWindow() {
    ...
    if (selectedPalette != "Choose a palette...") {
      ...
      var fileName =
        "pixel-" + currFile.canvasSize[0] + "x" + currFile.canvasSize[1];
  ...
  function saveProject() {
    ...
    if (typeof ga !== "undefined")
      ga(
        ...
        "Pixel Editor Save",
        selectedPalette,
        currFile.canvasSize[0] + "/" + currFile.canvasSize[1],
      ); /*global ga*/
  ...
  function exportProject() {
    if (EditorState.documentCreated()) {
      ...
      layersCopy.sort((a, b) =>
        a.canvas.style.zIndex > b.canvas.style.zIndex ? 1 : -1,
      );
      ...
      for (let i = 0; i < layersCopy.length; i++) {
        if (layersCopy[i].hasCanvas() && layersCopy[i].isVisible) {
          LayerList.mergeLayers(
            exportCanvas.getContext("2d"),
            layersCopy[i].context,
          );
        }
  ...
  function open() {
    ...
    if (EditorState.documentCreated()) {
      ...
      if (
        confirm(
          "Opening a pixel will discard your current one. Are you sure you want to do that?",
        )
      )
  ...
  function localStorageSave() {
    ...
    console.log(
      "BEFORE JSON.stringify(lpe.colors,null,4) === ",
      JSON.stringify(lpe.colors, null, 4),
    );
    console.log([...ColorModule.getCurrentPalette()]);
    if (lpe.colors.length < 1)
      lpe.colors = [...ColorModule.getCurrentPalette()];
    if (lpe.colors.length < 1) lpe.colors.push("#000000");
    console.log(
      "AFTER JSON.stringify(lpe.colors,null,4) === ",
      JSON.stringify(lpe.colors, null, 4),
    );
        ...
        layers: [
          {
            ...
            id: "layer0",
            name: "Layer 0",
            src: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAAXNSR0IArs4c6QAAAHZJREFUOE9jZKAQMFKon4EuBvxHcyWKpdhcgK4BpB+nS9ElYJqJ9hqyQpI1ozsNZABRNnMnNIEt+7qgjhGrBpgCWOCBFKJHN0gNTgOQFSPbhi5OlAHYEhpBL+DThO4tgoGGHB7YwgKvAbj8j+xCgi4glNkoNgAA3JApEbHObDkAAAAASUVORK5CYII=",
          },
          {
            canvas: {},
            context: { mozImageSmoothingEnabled: false },
            ...
            id: "layer1",
            name: "Layer 1",
            src: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAAXNSR0IArs4c6QAAAGNJREFUOE9jZKAQMCLrl21R/Q/iP665jSKOzw7qGgCyCeQKsl0AM4AUb2D1KymuoJ0BxHoDZ3QRG6V445uYsCBoACGvEExxhFxBlAH4XEHQAEKpkygDkFMoumuINgCWI9HDBAChJjwRzAXQUwAAAABJRU5ErkJggg==",
          },
          {
            canvas: {},
            context: { mozImageSmoothingEnabled: false },
            ...
            id: "layer2",
            name: "Layer 2",
            src: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAAXNSR0IArs4c6QAAAGNJREFUOE9jZKAQMJKi/4yk5H+YepPnz8F68RqArAFdI4yP1QBsNuFyKYYBMM0wJxLyIlYDiNWMNQxALhimBuCKUoKBSChKcRpASCPOhESsRoIGEBuVKF4g1XaMhERqMgYZAACIaEgR0hnFxgAAAABJRU5ErkJggg==",
          },
        ],
      }),
    );
  ...
  function loadFile() {
    ...
    if (browseHolder.files && browseHolder.files[0]) {
      ...
      if (extension == "png" || extension == "gif" || extension == "lpe") {
        ...
        alert(
          "Only .LPE project files, PNG and GIF files are allowed at this time.",
        );
    }
  ...
  function openProject(lpeData) {
    ...
    if (lpeData) {
      ...
      reader.onload = function (e) {
        ...
        console.log(
          "FileManager.js => openProject => loaded lpe dictionary === ",
          dictionary,
        );
  ...
  function getProjectData() {
    ...
    dictionary.colors = [...ColorModule.getCurrentPalette()];
    dictionary.layers = layersCopy.map((n, i) => {
  ...
  function loadPalette() {
    if (browsePaletteHolder.files && browsePaletteHolder.files[0]) {
      ...
      var fileContentType = file.type || file.name.split(".").slice(-1)[0];
      ...
      switch (fileContentType) {
        ...
        default:
          alert(
            "Only PNG, GIF, .hex and .gpl files are supported at this time.",
          );
      }
    }
  ...
  function loadPaletteFromImage(e) {
    ...
    img.onload = function () {
      ...
      var loadPaletteCanvas = document.getElementById(
        "load-palette-canvas-holder",
      );
      ...
      var colorPalette = [];
      var imagePixelData = loadPaletteContext.getImageData(
        0,
        0,
        this.width,
        this.height,
      ...
      for (var i = 0; i < imagePixelData.length; i += 4) {
        const newColor = {
          r: imagePixelData[i],
          g: imagePixelData[i + 1],
          b: imagePixelData[i + 2],
        };
  ...
  function openImportImageWindow() {
    ...
    if (!this.isImportWindowInitialized) {
      ...
      this.currentImportPivotElement = document.querySelector(
        ".import-image-location-pivot .rc-selected-pivot",
      );
      ...
      Events.on("click", "select-image", () =>
        document.getElementById("import-image-browse-holder")?.click(),
      );
  ...
  function importImage() {
    ...
    fileReader.onload = (e) => {
      ...
      img.onload = () => {
        let shouldResizeCanvas = document.getElementById(
          "import-image-match-size",
        ).checked;
        let shouldImportColors = document.getElementById(
          "import-image-update-palette",
        ).checked;

        // Resize the canvas to the image size if the flag was set to true.
        if (shouldResizeCanvas) {
          currFile.resizeCanvas(
            null,
        ...
        let offset = Util.getPivotPosition(
          this.currentImportPivotPosition,
          currFile.canvasSize[0],
          currFile.canvasSize[1],
          img.width,
          img.height,
  ...
  function loadImage() {
    if (importImageHolder.files && importImageHolder.files[0]) {
      ...
      document.getElementById("import-image-name").innerText =
        importImageHolder.files[0].name;

      // Checking if the extension is supported
      if (extension !== "png") {
        alert(
          "Only PNG files are currently allowed to be imported at this time.",
        );
        importImageHolder.value = null;
      }
    }
  ...
  document.getElementById("auto-cache-button").textContent =
    cacheBtnText(cacheEnabled);
