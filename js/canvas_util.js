...
function drawTinyTextOne(
  ...
  const CHARS = generateCharsFromFont(
    font,
    w,
    h,
    8,
...
function generateCharsFromFont(
  font,
  charW = 7,
  charH = 7,
  sampleScale = 8,
...
function pixelButton(
  ...
  yo,
  img,
  colors = ["#112", "#334", "#556", "#778", "#99A", "#BBC"],
) {
  const canvas = document.createElement("canvas");
  const w = img.width + 4;
...
function scaleImageData(imageData, scale) {
  ...
  var scaledImageData = document
    .createElement("canvas")
    .getContext("2d")
    .createImageData(imageData.width * scale, imageData.height * scale);
        ...
        for (var x = 0; x < scale; x++) {
          ...
          for (var i = 0; i < 4; i++) {
            scaledImageData.data[
              (destRow * scaledImageData.width + destCol) * 4 + i
            ] = sourcePixel[i];
          }
        }
      }
