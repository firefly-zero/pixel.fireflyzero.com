...
class Layer {
  ...
  constructor(width, height, canvas, menuEntry, id) {
    ...
    else if (menuEntry !== undefined) this.menuEntry = menuEntry;
    ...
    if (this.menuEntry !== undefined) {
      ...
      this.menuEntry.getElementsByTagName("button")[0].onclick = () =>
        this.toggleLock();
      this.menuEntry.getElementsByTagName("button")[1].onclick = () =>
  ...
  delete(layerIndex) {
    ...
    if (this.isSelected) {
      ...
      const nearestLayer =
        currFile.layers[layerIndex + 1] ?? currFile.layers[layerIndex - 1];
      if (nearestLayer) {
  ...
  tryDelete() {
    //TODO: quote yoda
    if (
      Input.getLastTarget() != this.menuEntry &&
      Input.getLastTarget().parentElement != this.menuEntry
    )
      return;
  ...
  initialize() {
    ...
    this.canvas.style.left =
      64 +
      currFile.canvasView.clientWidth / 2 -
      (currFile.canvasSize[0] * currFile.zoom) / 2 +
      "px";
    this.canvas.style.top =
      48 +
      currFile.canvasView.clientHeight / 2 -
      (currFile.canvasSize[1] * currFile.zoom) / 2 +
      "px";
  ...
  rename() {
    ...
    if (this.oldLayerName != null) {
      ...
      new HistoryState().RenameLayer(
        this.oldLayerName,
        name,
        currFile.currentLayer,
      );
      this.oldLayerName = null;
    }
  }
  ...
  setCanvasOffset(offsetLeft, offsetTop) {
    ...
    if (offsetLeft < minXOffset) this.canvas.style.left = minXOffset + "px";
    else if (offsetLeft > maxXOffset)
      this.canvas.style.left = maxXOffset + "px";
    else this.canvas.style.left = offsetLeft + "px";

    //vertical offset
    var minYOffset = -currFile.canvasSize[1] * currFile.zoom + 164;
    var maxYOffset = window.innerHeight - 100;

    if (offsetTop < minYOffset) this.canvas.style.top = minYOffset + "px";
    else if (offsetTop > maxYOffset) this.canvas.style.top = maxYOffset + "px";
    else this.canvas.style.top = offsetTop + "px";
  }
  ...
  selectLayer(hideOptions = true) {
    ...
    ////console.trace();
    if (hideOptions) LayerList.closeOptionsMenu();
    ...
    if (currFile.VFXLayer) {
      // only refresh z after init
      LayerList.refreshZ();
    }
  ...
  lock() {
    this.isLocked = true;
    this.menuEntry.getElementsByClassName("layer-button")[0].style.visibility =
      "visible";

    this.menuEntry.getElementsByClassName("default-icon")[0].style.display =
      "none";
    this.menuEntry.getElementsByClassName("edited-icon")[0].style.display =
      "inline-block";
  }
  ...
  show() {
    ...
    this.menuEntry.getElementsByClassName("layer-button")[1].style.visibility =
      "hidden";

    // Changing icon
    this.menuEntry.getElementsByClassName("default-icon")[1].style.display =
      "inline-block";
    this.menuEntry.getElementsByClassName("edited-icon")[1].style.display =
      "none";
  }
  ...
  updateLayerPreview() {
    ...
    destination
      .getContext("2d")
      .clearRect(0, 0, destination.width, destination.height);
    destination.getContext("2d").drawImage(
      this.canvas,
      // This is necessary to center the preview in the canvas
      (destination.width - previewWidth) / 2,
      (destination.height - previewHeight) / 2,
      previewWidth,
      previewHeight,
    );
  }
