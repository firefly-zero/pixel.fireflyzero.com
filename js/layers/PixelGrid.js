...
class PixelGrid extends Layer {
  ...
  enablePixelGrid() {
    if (!this.pixelGridVisible) return;
  ...
  togglePixelGrid(newState) {
    ...
    if (newState == "on") this.showPixelGrid();
    else if (newState == "off") this.hidePixelGrid();
    else if (this.pixelGridVisible) this.hidePixelGrid();
    else this.showPixelGrid();
  }
  ...
  fillPixelGrid() {
    ...
    for (
      let i = 0;
      i < this.canvas.width / Math.round(this.lineDistance);
      i++
    ) {
    ...
    for (
      let i = 0;
      i < this.canvas.height / Math.round(this.lineDistance);
      i++
    ) {
      this.context.beginPath();
      this.context.moveTo(0, i * Math.round(this.lineDistance) + 0.5);
      this.context.lineTo(
        originalSize[0] * Math.round(this.lineDistance),
        i * Math.round(this.lineDistance) + 0.5,
      );
      this.context.stroke();
      this.context.closePath();
