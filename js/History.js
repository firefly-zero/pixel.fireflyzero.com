...
const History = (() => {
  ...
  function saveHistoryState(state) {
    ...
    if (undoStates.length > Settings.getCurrSettings().numberOfHistoryStates) {
      undoStates = undoStates.splice(
        -Settings.getCurrSettings().numberOfHistoryStates,
        Settings.getCurrSettings().numberOfHistoryStates,
      );
    }
...
class HistoryState {
  ...
  FlattenTwoVisibles(
    ...
    this.undo = function () {
      ...
      LayerList.getLayerListEntries().insertBefore(
        aboveLayer.menuEntry,
        afterAbove,
      );

      belowLayer.context.clearRect(
        0,
        0,
  ...
  MergeLayer(aboveIndex, aboveLayer, belowData, belowLayer) {
    ...
    this.undo = function () {
      LayerList.getLayerListEntries().insertBefore(
        this.aboveLayer.menuEntry,
        this.belowLayer.menuEntry,
      );
  ...
  EditCanvas() {
    this.canvasState = currFile.currentLayer.context.getImageData(
      0,
      0,
      currFile.canvasSize[0],
      currFile.canvasSize[1],
    ...
    this.undo = function () {
      ...
      var currentCanvas = stateLayer.context.getImageData(
        0,
        0,
        currFile.canvasSize[0],
        currFile.canvasSize[1],
  ...
  DeleteColor(colorValue) {
    this.colorValue = colorValue;
    this.canvas = currFile.currentLayer.context.getImageData(
      0,
      0,
      currFile.canvasSize[0],
      currFile.canvasSize[1],
    ...
    this.undo = function () {
      var currentCanvas = currFile.currentLayer.context.getImageData(
        0,
        0,
        currFile.canvasSize[0],
        currFile.canvasSize[1],
  ...
  EditColor(newColorValue, oldColorValue) {
    ...
    this.undo = function () {
      let currentCanvas = currFile.currentLayer.context.getImageData(
        0,
        0,
        currFile.canvasSize[0],
        currFile.canvasSize[1],
