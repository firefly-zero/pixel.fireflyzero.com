class EllipseTool extends ResizableTool {
  ...
  constructor(name, options, switchFunction) {
    ...
    this.addTutorialKey(
      "S while selected",
      " to change fill mode (empty or fill)",
    );
  ...
  onStart(mousePos, mouseTarget) {
    ...
    currFile.TMPLayer.canvas.style.zIndex =
      parseInt(currFile.currentLayer.canvas.style.zIndex, 10) + 1;
  ...
  onDrag(mousePos) {
    ...
    this.drawEllipse(
      Math.floor(mousePos[0] / currFile.zoom) + 0.5,
      Math.floor(mousePos[1] / currFile.zoom) + 0.5,
  ...
  onEnd(mousePos) {
    ...
    if (this.startMousePos == undefined) return;
    ...
    if (this.currFillMode == "fill") {
      ...
      FillTool.fill(
        [
          this.startMousePos[0] * currFile.zoom,
          this.startMousePos[1] * currFile.zoom,
        ],
        currFile.TMPLayer.context,
      );
    }
    ...
    Util.pasteData(
      ...
      currFile.TMPLayer.context.getImageData(
        0,
        0,
        currFile.canvasSize[0],
        currFile.canvasSize[1],
    ...
    for (const coord in toFill) {
      ...
      if (
        !(
          // Top and left
          (
            (toFill[[arrayCoord[0], arrayCoord[1] - 1]] &&
              toFill[[arrayCoord[0] - 1, arrayCoord[1]]] &&
              !removed[[arrayCoord[0], arrayCoord[1] - 1]] &&
              !removed[(arrayCoord[0] - 1, arrayCoord[1])]) ||
            // Top and right
            (toFill[[arrayCoord[0], arrayCoord[1] - 1]] &&
              toFill[[arrayCoord[0] + 1, arrayCoord[1]]] &&
              !removed[[arrayCoord[0], arrayCoord[1] - 1]] &&
