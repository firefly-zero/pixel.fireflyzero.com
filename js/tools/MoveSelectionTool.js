class MoveSelectionTool extends DrawingTool {
  ...
  copySelection(event) {
    ...
    if (event) this.switchFunc(this.selectionTool);
  }

  cutSelection(event) {
    if (currFile.currentLayer.isLocked) return;
    this.cutting = true;
    this.lastCopiedSelection = this.currSelection;
    this.endSelection();
  ...
  pasteSelection() {
    if (currFile.currentLayer.isLocked) return;
    if (this.lastCopiedSelection === undefined) return;
    if (
      !(
        this.currMousePos[0] / currFile.zoom >= 0 &&
        this.currMousePos[1] / currFile.zoom >= 0 &&
        this.currMousePos[0] / currFile.zoom < currFile.canvasSize[0] &&
        this.currMousePos[1] / currFile.zoom < currFile.canvasSize[1]
      )
    )
      this.currMousePos = [
  ...
  onDrag(mousePos) {
    ...
    this.selectionTool.moveOffset = [
      Math.floor(
        mousePos[0] / currFile.zoom -
          currFile.canvasSize[0] / 2 -
          (this.selectionTool.boundingBoxCenter[0] -
            currFile.canvasSize[0] / 2),
      ),
      Math.floor(
        mousePos[1] / currFile.zoom -
          currFile.canvasSize[1] / 2 -
          (this.selectionTool.boundingBoxCenter[1] -
            currFile.canvasSize[1] / 2),
      ),
    ];
    ...
    currFile.TMPLayer.context.clearRect(
      0,
      0,
      currFile.TMPLayer.canvas.width,
      currFile.TMPLayer.canvas.height,
    );
    // put the image data on the tmp layer with offset
    currFile.TMPLayer.context.putImageData(
      this.currSelection,
      this.selectionTool.moveOffset[0],
      this.selectionTool.moveOffset[1],
    );
  ...
  onEnd(mousePos, mouseTarget) {
    ...
    if (
      !this.selectionTool.cursorInSelectedArea(mousePos) &&
