class EyeDropperTool extends Tool {
  ...
  onDrag(mousePos) {
    ...
    this.eyedropperPreview.style.borderColor =
      "#" + Color.rgbToHex(this.selectedColor);
    this.eyedropperPreview.style.left =
      mousePos[0] + currFile.currentLayer.canvas.offsetLeft - 30 + "px";
    this.eyedropperPreview.style.top =
      mousePos[1] + currFile.currentLayer.canvas.offsetTop - 30 + "px";

    const colorLightness = Math.max(
  ...
  pickColor(cursorLocation) {
    ...
    for (let i = 0; i < currFile.layers.length; i++) {
      ...
      tmpColour = currFile.layers[i].context.getImageData(
      ...
      if (
        currFile.layers[i].canvas.style.zIndex > max ||
        Util.isPixelEmpty(selectedColor) ||
