class MagicWandTool extends SelectionTool {
  ...
  getSelection() {
    let coords = [
      Math.floor(this.endMousePos[0]),
      Math.floor(this.endMousePos[1]),
    ];
    let data = currFile.currentLayer.context.getImageData(
      0,
      0,
    ...
    let selectedData = new ImageData(
      currFile.canvasSize[0],
      currFile.canvasSize[1],
    );

    this.visit(
      [Math.floor(this.endMousePos[0]), Math.floor(this.endMousePos[1])],
      this.currSelection,
      data,
      color,
    );
    ...
    for (const pixel in this.currSelection) {
      let coords = [
        parseInt(pixel.split(",")[0]),
        parseInt(pixel.split(",")[1]),
      ];
    ...
    this.outlineData = new ImageData(
      currFile.canvasSize[0],
      currFile.canvasSize[1],
    );
  ...
  visit(pixel, selected, data, color) {
    ...
    while (toVisit.length > 0) {
      ...
      if (right != undefined && visited[right] == undefined)
        toVisit.push(right);
      if (left != undefined && visited[left] == undefined) toVisit.push(left);
      if (top != undefined && visited[top] == undefined) toVisit.push(top);
