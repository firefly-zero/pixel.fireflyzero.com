class FillTool extends DrawingTool {
  ...
  onStart(mousePos, target) {
    ...
    if (target.className != "drawingCanvas") return;
  ...
  static fill(cursorLocation, context) {
    ...
    if (context == undefined) context = currFile.currentLayer.context;

    //temporary image holds the data while we change it
    let tempImage = context.getImageData(
      0,
      0,
    ...
    let topmostPixelsArray = [
      [
        Math.floor(cursorLocation[0] / currFile.zoom),
        Math.floor(cursorLocation[1] / currFile.zoom),
      ],
    ];
    ...
    let startingPosition =
      (topmostPixelsArray[0][1] * currFile.canvasSize[0] +
        topmostPixelsArray[0][0]) *
      4;

    //the color of the cluster that is being filled
    let clusterColor = [
      tempImage.data[startingPosition],
      tempImage.data[startingPosition + 1],
      tempImage.data[startingPosition + 2],
      tempImage.data[startingPosition + 3],
    ];
    ...
    while (topmostPixelsArray.length) {
      ...
      while (
        y++ < currFile.canvasSize[1] - 1 &&
        matchStartColor(tempImage, pixelPos, clusterColor)
      ) {
