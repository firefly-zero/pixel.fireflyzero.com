class SelectionTool extends Tool {
  ...
  onStart(mousePos, mouseTarget) {
    ...
    if (
      ...
      !Util.cursorInCanvas(currFile.canvasSize, [
        mousePos[0] / currFile.zoom,
        mousePos[1] / currFile.zoom,
      ])
    )
      return;
  ...
  onDrag(mousePos) {
    ...
    if (mouseX > currFile.canvasSize[0]) this.reconstruct.right = true;
    else if (mouseX < 0) this.reconstruct.left = true;

    if (mouseY > currFile.canvasSize[1]) this.reconstruct.bottom = true;
    else if (mouseY < 0) this.reconstruct.top = true;
  ...
  onEnd(mousePos, mouseTarget) {
    ...
    this.boundingBoxCenter = [
      this.boundingBox.minX +
        (this.boundingBox.maxX - this.boundingBox.minX) / 2,
  ...
  cutSelection() {
    let currLayerData = currFile.currentLayer.context.getImageData(
      0,
      0,
    ...
    currFile.currentLayer.context.putImageData(
      new ImageData(currLayerData, currFile.canvasSize[0]),
      0,
      0,
    );
  }
  ...
  pasteSelection() {
    if (this.currSelection == undefined) return;
    ...
    let pasteData = currFile.TMPLayer.context.getImageData(
      0,
      0,
      currFile.canvasSize[0],
      currFile.canvasSize[1],
    ...
    currFile.TMPLayer.context.clearRect(
      0,
      0,
      currFile.canvasSize[0],
      currFile.canvasSize[1],
    );
    currFile.VFXLayer.context.clearRect(
      0,
      0,
      currFile.canvasSize[0],
      currFile.canvasSize[1],
  ...
  cursorInSelectedArea(mousePos) {
    let floored = [
      Math.floor(mousePos[0] / currFile.zoom) - this.moveOffset[0],
      Math.floor(mousePos[1] / currFile.zoom) - this.moveOffset[1],
    ];

    if (this.currSelection[floored] != undefined) return true;
    return false;
  }
  ...
  visit(pixel, visited, data) {
    ...
    while (toVisit.length > 0) {
      ...
      let col = Util.getPixelColor(
        data,
        pixel[0],
        pixel[1],
        currFile.canvasSize[0],
      );
      if (col[3] == 255) continue;
      if (this.isBorderOfBox(pixel)) return [];

      let top, bottom, left, right;
      if (pixel[1] > 0) top = [pixel[0], pixel[1] - 1];
      else top = undefined;

      if (pixel[0] > 0) left = [pixel[0] - 1, pixel[1]];
      else left = undefined;

      if (pixel[1] < currFile.canvasSize[1]) bottom = [pixel[0], pixel[1] + 1];
      else bottom = undefined;

      if (pixel[0] < currFile.canvasSize[0]) right = [pixel[0] + 1, pixel[1]];
      ...
      if (left != undefined && currVisited[left] == undefined)
        toVisit.push(left);
      if (top != undefined && currVisited[top] == undefined) toVisit.push(top);
  ...
  getSelection() {
    let selected = [];
    let visited = {};
    let data = currFile.VFXLayer.context.getImageData(
      0,
      0,
      currFile.canvasSize[0],
      currFile.canvasSize[1],
    ...
    for (
      let x = this.boundingBox.minX - 1;
      x <= this.boundingBox.maxX + 1;
      x++
    ) {
      for (
        let y = this.boundingBox.minY - 1;
        y <= this.boundingBox.maxY + 1;
        y++
      ) {
    ...
    this.outlineData = currFile.VFXLayer.context.getImageData(
      this.boundingBox.minX,
      this.boundingBox.minY,
  ...
  drawOutline() {
    ...
    currFile.VFXLayer.context.putImageData(
      this.outlineData,
      this.boundingBox.minX + this.moveOffset[0],
  ...
  isBorderOfBox(pixel) {
    return (
      pixel[0] == this.boundingBox.minX ||
      pixel[0] == this.boundingBox.maxX ||
      pixel[1] == this.boundingBox.minY ||
      pixel[1] == this.boundingBox.maxY
    );
  }
