class ZoomTool extends Tool {
  ...
  onMouseWheel(mousePos, mode) {
    ...
    if (mode == "out" && currFile.zoom > MIN_ZOOM_LEVEL) {
      zoomed = true;
      if (currFile.zoom > 2) currFile.zoom -= Math.ceil(currFile.zoom / 10);
      else currFile.zoom -= Math.ceil((currFile.zoom * 2) / 10) / 2;
      ...
      currFile.layers[0].setCanvasOffset(
        currFile.layers[0].canvas.offsetLeft +
          ((oldWidth - newWidth) * mousePos[0]) / oldWidth,
        currFile.layers[0].canvas.offsetTop +
          ((oldHeight - newHeight) * mousePos[1]) / oldHeight,
      );
    }
    //if you want to zoom in
    else if (
      mode == "in" &&
      currFile.zoom + Math.ceil(currFile.zoom / 10) < window.innerHeight / 4
    ) {
      zoomed = true;
      if (currFile.zoom > 2) currFile.zoom += Math.ceil(currFile.zoom / 10);
      else {
      ...
      currFile.layers[0].setCanvasOffset(
        currFile.layers[0].canvas.offsetLeft -
          Math.round(((newWidth - oldWidth) * mousePos[0]) / oldWidth),
        currFile.layers[0].canvas.offsetTop -
          Math.round(((newHeight - oldHeight) * mousePos[1]) / oldHeight),
      );
    }
    ...
    if (zoomed) {
      if (currFile.zoom <= 7) currFile.pixelGrid.disablePixelGrid();
