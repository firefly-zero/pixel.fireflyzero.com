class LassoSelectionTool extends SelectionTool {
  ...
  drawSelection() {
    if (this.currentPixels.length <= 1) return;
    let point = [];
    let prevPoint = [];
    ...
    for (var index = 0; index < this.currentPixels.length; index++) {
      ...
      if (index == 0) currFile.VFXLayer.context.moveTo(point[0], point[1]);
      else {
        prevPoint = this.currentPixels[index - 1];
        currFile.VFXLayer.drawLine(
          Math.floor(prevPoint[0]),
          Math.floor(prevPoint[1]),
          Math.floor(point[0]),
          Math.floor(point[1]),
          1,
        );
    ...
    currFile.VFXLayer.drawLine(
      Math.floor(point[0]),
      Math.floor(point[1]),
      Math.floor(this.currentPixels[0][0]),
      Math.floor(this.currentPixels[0][1]),
      1,
    );
  }
}
