class ResizableTool extends DrawingTool {
  ...
  onSelect(mousePos) {
    ...
    if (this.toolSizeInput == undefined) {
      this.toolSizeInput = InputComponents.createNumber(
        this.name + "-input",
        "Tool size",
      );
      Events.on(
        "change",
        this.toolSizeInput.getElementsByTagName("input")[0],
  ...
  onRightDrag(mousePos, mouseEvent) {
    ...
    let distanceFromClick =
      mousePos[0] / currFile.zoom - this.startResizePos[0] / currFile.zoom;
