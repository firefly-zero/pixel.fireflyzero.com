const TopMenuModule = (() => {
  ...
  let infoList =
    document.getElementById("editor-info") ?? document.createElement("div");
  let infoElements = {};
  ...
  function initMenu() {
    ...
    for (let i = 1; i < mainMenuItems.length - 1; i++) {
      ...
      for (var j = 0; j < subMenuItems.length; j++) {
        ...
        switch (currSubmenuButton.textContent) {
          ...
          case "Save project":
            Events.on(
              "click",
              currSubmenuButton,
              FileManager.openSaveProjectWindow,
            );
            break;
          case "Open":
          ...
          case "Import":
            Events.on(
              "click",
              currSubmenuButton,
              FileManager.openImportImageWindow,
            );
            break;
          case "Load palette":
            Events.on(
              "click",
              currSubmenuButton,
              FileManager.openImportPaletteWindow,
            );
            break;
          case "Export":
            Events.on(
              "click",
              currSubmenuButton,
              FileManager.openPixelExportWindow,
            );
            break;
          case "Exit":
            ...
            if (EditorState.documentCreated()) {
              ...
              if (
                confirm(
                  "Exiting will discard your current pixel. Are you sure you want to do that?",
                )
              )
        ...
        Events.on("click", currSubmenuButton, function () {
