class File {
  ...
  zoom = 7;
  canvasView =
    document.getElementById("canvas-view") ?? document.createElement("div");
  inited = false;
  ...
  rcChangedBorder() {
    ...
    document.getElementById("rc-width").value =
      parseInt(currFile.canvasSize[0]) +
      this.rcBorders.left +
      this.rcBorders.right;
    document.getElementById("rc-height").value =
      parseInt(currFile.canvasSize[1]) +
      this.rcBorders.top +
      this.rcBorders.bottom;
  }
  ...
  rcChangedSize() {
    let widthOffset =
      Math.abs(document.getElementById("rc-width").value) -
      currFile.canvasSize[0];
    let heightOffset =
      Math.abs(document.getElementById("rc-height").value) -
  ...
  resizeCanvas(event, size, customData, saveHistory = true) {
    ...
    for (let i = 0; i < currFile.layers.length; i++) {
      imageDatas.push(
        currFile.layers[i].context.getImageData(
          0,
          0,
          currFile.canvasSize[0],
          currFile.canvasSize[1],
    ...
    if (saveHistory && event != null) {
      ...
      new HistoryState().ResizeCanvas(
        ...
        { x: currFile.canvasSize[0], y: currFile.canvasSize[1] },
        imageDatas.slice(),
        customData != null && saveHistory,
      );
    }
    ...
    for (let i = 0; i < currFile.layers.length; i++) {
      if (customData == undefined) {
        currFile.layers[i].context.putImageData(
          imageDatas[copiedDataIndex],
          leftOffset,
          topOffset,
        );
      } else {
        currFile.layers[i].context.putImageData(
          customData[copiedDataIndex],
          0,
          0,
        );
      }
  ...
  trimCanvas(event, saveHistory) {
    ...
    for (let i = 0; i < currFile.layers.length; i++) {
      imageDatas.push(
        currFile.layers[i].context.getImageData(
          minX - 1,
          currFile.layers[i].canvasSize[1] - maxY,
          maxX - minX + 1,
          maxY - minY + 1,
        ),
      );
  ...
  rcUpdateBorders() {
    ...
    // Validating input
    this.rcBorders.left == ""
      ? (this.rcBorders.left = 0)
      : (this.rcBorders.left = Math.round(parseInt(this.rcBorders.left)));
    this.rcBorders.right == ""
      ? (this.rcBorders.right = 0)
      : (this.rcBorders.right = Math.round(parseInt(this.rcBorders.right)));
    this.rcBorders.top == ""
      ? (this.rcBorders.top = 0)
      : (this.rcBorders.top = Math.round(parseInt(this.rcBorders.top)));
    this.rcBorders.bottom == ""
      ? (this.rcBorders.bottom = 0)
      : (this.rcBorders.bottom = Math.round(parseInt(this.rcBorders.bottom)));
  }
  ...
  initResizeSpriteInputs() {
    ...
    Events.on(
      "change",
      "rs-width-percentage",
      this.changedWidthPercentage.bind(this),
    );
    Events.on(
      "change",
      "rs-height-percentage",
      this.changedHeightPercentage.bind(this),
    );
    ...
    Events.on(
      "change",
      "resize-algorithm-combobox",
      this.changedAlgorithm.bind(this),
    );
  }
  ...
  changedHeightPercentage(event) {
    ...
    if (this.keepRatio) {
      document.getElementById("rs-width-percentage").value =
        this.data.heightPercentage * currentRatio;
      this.data.widthPercentage = newWidthPerc;
  ...
  resizeImageData(image, width, height, algorithm) {
    ...
    switch (algorithm) {
      ...
      case "bilinear-interpolation":
        resize = this.bilinearInterpolation;
        break;
      default:
        return image;
    }
