const LayerList = (() => {
  ...
  function mergeLayers(belowLayer, topLayer) {
    ...
    let belowImageData = belowLayer.getImageData(
      0,
      0,
      belowLayer.canvas.width,
      belowLayer.canvas.height,
    );
    let toMergeImageData = topLayer.getImageData(
      0,
      0,
      topLayer.canvas.width,
      topLayer.canvas.height,
    );

    for (let i = 0; i < belowImageData.data.length; i += 4) {
      let currentMovePixel = [
        toMergeImageData.data[i],
        toMergeImageData.data[i + 1],
        toMergeImageData.data[i + 2],
        toMergeImageData.data[i + 3],
      ];

      let currentUnderlyingPixel = [
        //TODO: I'd be curious to know if this array slows this function down
        belowImageData.data[i],
        belowImageData.data[i + 1],
        belowImageData.data[i + 2],
        belowImageData.data[i + 3],
      ];
  ...
  function layerDragEnd(event) {
    ...
    let selectedId;
    const idArr = [...document.querySelectorAll(".layers-menu-entry")].map(
      (elm) => {
  ...
  function getLayerByName(name) {
    for (let i = 0; i < currFile.layers.length; i++) {
      if (currFile.layers[i].hasCanvas()) {
        if (
          currFile.layers[i].menuEntry.getElementsByTagName("p")[0].innerHTML ==
          name
        ) {
  ...
  function duplicateLayer(event, saveHistory = true) {
    ...
    newLayer.context.putImageData(
      currFile.currentLayer.context.getImageData(
        0,
        0,
        currFile.canvasSize[0],
        currFile.canvasSize[1],
  ...
  function merge(saveHistory = true) {
    ...
    // Getting layer below
    let layerBelow = LayerList.getLayerByID(
      currFile.currentLayer.menuEntry.nextElementSibling.id,
    );
    ...
    if (layerBelow != null) {
      ...
      if (saveHistory) {
        new HistoryState().MergeLayer(
          toMergeIndex,
          toMerge,
          layerBelow.context.getImageData(
            0,
            0,
            currFile.canvasSize[0],
            currFile.canvasSize[1],
    ...
    if (!onlyVisible) {
      ...
      for (let i = 0; i < visibleLayers.length - 1; i++) {
        ...
        new HistoryState().FlattenTwoVisibles(
          visibleLayers[i + 1].context.getImageData(
            0,
            0,
            currFile.canvasSize[0],
            currFile.canvasSize[1],
        ...
        LayerList.mergeLayers(
          visibleLayers[i + 1].context,
          visibleLayers[i].context,
        );
  ...
  function openOptionsMenu(event) {
    if (event.which == 3) {
      ...
      while (
        ...
        !target.classList.contains("layers-menu-entry")
      ) {
