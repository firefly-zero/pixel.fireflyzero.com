...
class Util {
  ...
  static pasteData(underlyingImageData, pasteData, finalContext) {
    for (let i = 0; i < underlyingImageData.data.length; i += 4) {
      ...
      let currentUnderlyingPixel = [
        underlyingImageData.data[i],
        underlyingImageData.data[i + 1],
        underlyingImageData.data[i + 2],
        underlyingImageData.data[i + 3],
      ];
  ...
  static cursorInCanvas(canvasSize, mousePos) {
    return (
      mousePos[0] >= 0 &&
      mousePos[1] >= 0 &&
      canvasSize[0] > mousePos[0] &&
      canvasSize[1] > mousePos[1]
    );
  }
