const PaletteBlock = (() => {
  ...
  function init() {
    ...
    for (let i = 0; i < simplePalette.childElementCount - 1; i++) {
      addSingleColour(
        Color.cssToHex(
          simplePalette.children[i].children[0].style.backgroundColor,
        ),
      );
    }
  }
  ...
  function updateRampSelection(mouseEvent) {
    ...
    if (mouseEvent == null || mouseEvent.buttons == 2) {
      ...
      for (let i = startIndex; i <= endIndex; i++) {
        ...
        if (
          currentCoords[1] == startCoords[1] ||
          (currentCoords[1] == startCoords[1] + 1 &&
            currentCoords[0] < startCoords[0])
        ) {
          bordersToSet.push("border-top");
        }
        if (
          currentCoords[1] == endCoords[1] ||
          (currentCoords[1] == endCoords[1] - 1 &&
            currentCoords[0] > endCoords[0])
        ) {
          bordersToSet.push("border-bottom");
        }
        if (
          i == coloursList.childElementCount - 1 ||
          currentCoords[0] ==
            Math.floor(blockData.blockWidth / blockData.squareSize) - 1 ||
          i == endIndex
        ) {
  ...
  function endRampSelection(mouseEvent) {
    ...
    if (currentSelection.startCoords.length == 0) {
      ...
      currentSelection.startCoords = getColourCoordinates(
    ...
    currentSelection.endCoords = getColourCoordinates(
      getElementIndex(mouseEvent.target),
    );

    // Setting the colour in the colour picker
    col = Color.cssToHex(
      coloursList.children[currentSelection.startIndex].style.backgroundColor,
    );
  ...
  function getColourCoordinates(index) {
    let yIndex = Math.floor(
      index / Math.floor(blockData.blockWidth / blockData.squareSize),
    );
